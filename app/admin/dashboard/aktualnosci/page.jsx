...
export default function Home() {
  ...
  return (
    ...
    <section className='w-full min-h-[500px] sm:px-10 px-5 pt-16 sm:pb-10 pb-5 relative drop-shadow-md bg-[#eee] flex justify-start items-center flex-col sm:gap-10 gap-8'>
      ...
      <div className="sm:w-[80%] w-full h-auto flex sm:justify-start justify-center items-center">
        ...
        <section className="sm:w-auto w-full flex flex-col justify-start items-center">
          <div className="w-full flex justify-between items-center gap-5 font-klein text-[#705555] sm:text-sm text-xs">
            <div className="w-full overflow-hidden flex px-5 justify-start items-center gap-5">
              <p className="2xl:min-w-[300px] min-w-[200px] 2xl:max-w-[300px] max-w-[200px] overflow-hidden whitespace-nowrap text-ellipsis px-2">Tytuł artykułu:</p>
              <p className="min-w-[200px] max-w-[200px] overflow-hidden whitespace-nowrap text-ellipsis sm:flex hidden px-2">Podtytuł:</p>
              <p className="min-w-[200px] max-w-[200px] overflow-hidden whitespace-nowrap text-ellipsis 2xl:flex hidden px-2">Zdjęcie:</p>
            </div>
          ...
          {artData.map((data, index) => (
            <div key={index} className="w-full flex justify-between items-center gap-2 mb-4">
              <div className="w-full overflow-hidden flex justify-start items-center py-1 sm:px-5 px-2 bg-[#fff] rounded-md gap-5 font-klein sm:text-base text-sm">
                ...
                <p className="min-w-[200px] max-w-[200px] overflow-hidden whitespace-nowrap text-ellipsis sm:flex hidden bg-[#eee] px-2 py-1 rounded-lg">{data.podtytul ? data.podtytul : "-"}</p>
                <p className="min-w-[200px] max-w-[200px] overflow-hidden whitespace-nowrap text-ellipsis 2xl:flex hidden bg-[#eee] px-2 py-1 rounded-lg">{data.zdjecie ? data.zdjecie : "-"}</p>
              </div>
    ...
    {isAddModalOpen &&
      <AddArticleModal closeModal={() => setIsAddModalOpen(false)}/>
    }
    {isEditModalOpen &&
      <EditArticleModal closeModal={() => setIsEditModalOpen(false)} props={clickedSlide}/>
    }
    </>
  );
}