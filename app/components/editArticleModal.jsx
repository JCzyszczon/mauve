...
function EditArticleModal({ closeModal, props, tableN }) {
  ...
  const [subtitle, setSubtitle] = useState(props.podtytul);
  const [buttonText, setButtonText] = useState(props.przycisk);
  const [buttonLink, setButtonLink] = useState(props.linkPrzycisk);
  ...
  useEffect(() => {
    setInputText(paragrafyToString(props.paragrafy));
  }, [props]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (error) {
      ...
      setMessage('Pomyślnie edytowano artykuł');
      setModalState(true);
      setFailure(false);
